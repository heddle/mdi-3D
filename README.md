...
The kinetics demo (`view3D.kineticsDemo`) simulates elastic collisions among
50,000 particles released from one corner of a box, with entropy tracked in
real time in a synchronized 2D strip chart (`EntropyPlotPanel`). Below: the
low-entropy initial state (left) and the equilibrated final state (right).
...
This is a good example of the coexistence this package is built around: 3D
rendering, a `SimulationEngine`-driven physics model, and 2D `sPlot` plotting
all running together in one MDI application.
